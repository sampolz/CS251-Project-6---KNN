    ...
    for root, dirs, files in os.walk(email_path):
        for file in files:
            ...
            with open(file_path, 'r', encoding='latin1') as f:
                ...
                words = tokenize_words(email_content)
                for word in words:
...
def find_top_words(word_freq, num_features=200):
    ...
    top_words = [word for word, count in sorted_words[:num_features]]
    counts = [count for word, count in sorted_words[:num_features]]
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron'):
    ...
    for root, dirs, files in os.walk(email_path):
        for index, file in enumerate(files):
            feature_per_email = np.zeros(len(top_words))
            if '.DS_Store' in file:
                continue
            else:
                if 'ham' in root:
                    ...
                    for line in content:
                        ...
                        for word in words:
                            if word in top_words:
                                feature_per_email[top_words.index(word)] += 1
            feats.append(feature_per_email)
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    test_size = int(test_prop * len(features))
    x_train, x_test = features[:-test_size], features[-test_size:]
    y_train, y_test = y[:-test_size], y[-test_size:]
    inds_train, inds_test = inds[:-test_size], inds[-test_size:]
